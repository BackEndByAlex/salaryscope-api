...
1. Validates that the `state` parameter is non-empty (login-CSRF protection — the client verifies state before calling this mutation, the server validates it is present and non-empty)
...
3. Uses the returned access token to fetch the user's profile from `https://www.googleapis.com/oauth2/v3/userinfo`
...
The JWT is never returned in the response body — all four auth mutations deliver it exclusively via the HttpOnly cookie.
...
The types and operations (`User`, `AuthPayload`, `register`, `login`, `githubLogin`, `googleLogin`, `logout`, `me`) are defined in `src/graphql/schema/auth.graphql`.
